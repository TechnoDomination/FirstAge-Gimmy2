...
public class GoBildaPinPointDriver extends I2cDeviceSynchDevice<I2cDeviceSynchSimple> {
    ...
    //Variables for showing where you are on the field
    private int deviceStatus = 0, loopTime = 0, xEncoderValue = 0, yEncoderValue = 0;
    private float xPosition = 0, yPosition = 0, hOrientation = 0, xVelocity = 0, yVelocity = 0, hVelocity = 0;
    ...
    public GoBildaPinPointDriver(I2cDeviceSynchSimple deviceClient, boolean deviceClientIsOwned) {
        ...
        //setting address for the pinpoint device
        this.deviceClient.setI2cAddress(I2cAddr.create7bit(DEFAULT_ADDRESS));
        //preparing device to see lifecycle events (start/stop)
        super.registerArmingStateCallback(false);
    }

    //Accessing the LynxI2cDeviceSynch (by casting) and setting the speed
    @Override
    ...
    //Register map of the i2c device (blueprint)
    ...
    //captures current default condition of the device
    ...
    //type of odometry pods (On gobuilda)
    ...
    /** Writes an int to the i2c device
     @param reg the register to write the int to - from enum Register
     ...
     * @param reg the register to read from - from enum register
    ...
    /**
     * Converts a byte array to a float value (decimal)
    ...
    /**
     * Reads a float (decimal) from a register
    ...
    /**
     * reading status of i2c device
     * Looks up the DeviceStatus enum corresponding with an int value (param s)
    ...
    /**
     * Sets odometry pod positions relative to center of robot
     * x offset is how far from center right/left, and y offset is forward/backward
